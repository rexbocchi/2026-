...
function restoreView(){
  ...
  const validModes = ['practice','recite','wrong','favorite'];
  mode = validModes.includes(view.mode) ? view.mode : 'practice';
  document.querySelectorAll('.tab').forEach(btn=>btn.classList.toggle('active', btn.dataset.mode===mode));
  if(['all','single','multiple','judge','short','case'].includes(view.type)) $('typeFilter').value=view.type;
  if(['order','random'].includes(view.order)) $('orderFilter').value=view.order;
  if(typeof view.search === 'string') $('searchInput').value=view.search;
}
function persistView(q){
  store.view = {mode, type:$('typeFilter').value, order:$('orderFilter').value, search:$('searchInput').value, lastQuestionId:q?.id || null};
  localStorage.setItem(STORE_KEY, JSON.stringify(store));
}

function applyFilters(restoreLast=false){
  ...
  if(restoreLast && store.view?.lastQuestionId){
    const savedIndex = list.findIndex(q=>q.id===store.view.lastQuestionId);
    current = savedIndex >= 0 ? savedIndex : Math.min(current, Math.max(0, list.length-1));
  }else current = Math.min(current, Math.max(0, list.length-1));
  render();
}
function render(){ selected=new Set(); answered=false; const q=list[current]; updateProgress();
  if(!q){ $('questionCard').innerHTML=`<div class="empty">暂无题目。可以切换题型或模式试试。</div>`; $('favBtn').textContent='☆ 收藏'; updateBottomAction(); return; }
  ...
  wireQuestion(q);
  updateBottomAction(q);
  persistView(q);
}
function practiceHtml(q){
  const actions = `<div class="actions"><button class="action-btn" id="showBtn">${q.type==='short'||q.type==='case'?'查看参考答案':'查看答案'}</button><button class="action-btn" id="skipBtn">跳过本题</button></div><div id="feedback"></div>`;
  if(q.type==='single' || q.type==='multiple') return `<div class="options">${q.options.map(o=>optHtml(o,false)).join('')}</div>${actions}`;
  if(q.type==='judge') return `<div class="judge-buttons"><button class="option" data-key="对"><span class="option-key">对</span></button><button class="option" data-key="错"><span class="option-key">错</span></button></div>${actions}`;
  return actions;
}
...
function wireQuestion(q){
  ...
  const show=$('showBtn'), skip=$('skipBtn'); if(show) show.onclick=()=>showAnswer(q); if(skip) skip.onclick=()=>go(1);
}
...
function handlePrimaryAction(){
  ...
  if(!q) return;
  if(mode==='recite' || answered) return go(1);
  if(q.type==='single' || q.type==='multiple' || q.type==='judge') return submitAnswer(q);
  showAnswer(q);
}
